...
namespace gws {
	...
	void SurvivalSystem::update() {
		...
		for(auto i = 0; i < end; ++i){
			...
			SurvivalStrategy stype;
			const PositionComponent* this_pos = &(world.positions[i]);
			const PositionComponent* north;
			const PositionComponent* northeast;
			const PositionComponent* east;
			const PositionComponent* southeast;
			const PositionComponent* south;
			const PositionComponent* southwest;
			const PositionComponent* west;
			const PositionComponent* northwest;
			//tbb::concurrent_vector<const PositionComponent*>::iterator &(*pos_iter);
			...
			int nutrientReq = world.nutrients[i].nutrientLevel;
			int waterReq    = world.waters[i].waterLevel;
			switch(stype){
				case EXPLORE:
	...
	int SurvivalSystem::tryRandom(int start_id){
		//SurvivalStrategy stype = world.survivors[start_id].strategy;
		const const PositionComponent* this_pos = &(world.positions[start_id]);
		const PositionComponent* north;
		const PositionComponent* northeast;
		const PositionComponent* east;
		const PositionComponent* southeast;
	...
	int SurvivalSystem::tryClockwise(int start_id){
		//SurvivalStrategy stype = world.survivors[start_id].strategy;
		const PositionComponent* this_pos = &(world.positions[start_id]);
		const PositionComponent* north;
		const PositionComponent* northeast;
		const PositionComponent* east;
		const PositionComponent* southeast;
